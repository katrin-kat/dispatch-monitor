...
def _resolve_missed_call(calls_ws, customer_phone, new_call_timestamp):
    ...
    for idx, row in enumerate(records, start=2):
        # Ищем либо пропущенный звонок (answered=False), либо звонок,
        # на который ответил автоответчик (voicemail=True) — в обоих случаях
        # клиент ещё не поговорил с живым человеком, resolved=False
        was_unresolved_contact = (
            str(row["answered"]) == "False" or str(row["voicemail"]) == "True"
        )
        if (
...
def performance_report():
    ...
    for c in calls:
        ...
        if c["direction"] == "outbound" and str(c["answered"]) == "True":
    ...
    still_not_reached = sum(
        1 for c in calls
        if (str(c["answered"]) == "False" or str(c["voicemail"]) == "True")
        and str(c["resolved"]) == "False"
    )
