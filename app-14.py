...
def housecallpro_appointment_webhook():
    ...
    ws.append_row([
        ...
        customer_name, dispatcher_name, technician_name, event_type, job_status
    ])

    return jsonify({"status": "ok"}), 200


def _resolve_dispatcher_by_recent_call(calls_ws, customer_phone, before_time_str, window_hours=24):
    """Ищет диспетчера по недавнему звонку с тем же номером телефона клиента,
