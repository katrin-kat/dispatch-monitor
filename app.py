...
def daily_summary():
    ...
    for agent, s in stats.items():
        ...
        ordered.append((agent, s, answer_rate, booking_rate, avg_duration))
    ...
    for agent, s, answer_rate, booking_rate, avg_duration in ordered:
        total_calls_agent = s["answered"] + s["missed"]
        lines.append(
            f"— {agent}: {total_calls_agent} calls, {answer_rate}% answered, "
            f"avg {_format_duration(avg_duration)}, {s['appointments']} booked ({booking_rate}%), "
            f"{s.get('requested', 0)} requested"
        )
    if not stats:
