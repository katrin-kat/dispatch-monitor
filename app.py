...
def performance_report():
    ...
    for ap in appointments:
        ...
        if name not in KNOWN_DISPATCHERS:
            ...
            continue
        if ap["event_type"] not in ("job.scheduled", "job.appointment.scheduled", "job.appointment.rescheduled"):
...
def daily_summary():
    ...
    for a in today_appointments:
        ...
        if agent not in KNOWN_DISPATCHERS:
            continue
        if a["event_type"] not in ("job.scheduled", "job.appointment.scheduled", "job.appointment.rescheduled"):
            continue
        stats.setdefault(agent, {"answered": 0, "missed": 0, "sms_answered": 0, "sms_missed": 0, "appointments": 0})
...
def dispatcher_report():
    ...
    her_appointments = [
        ...
        and a["event_type"] in ("job.scheduled", "job.appointment.scheduled", "job.appointment.rescheduled")
    ]
