...
CALLRAIL_API_KEY = os.environ.get("CALLRAIL_API_KEY")  # ключ доступа из CallRail -> Integrations -> API Keys
CALLRAIL_ACCOUNT_ID = os.environ.get("CALLRAIL_ACCOUNT_ID")  # ID аккаунта CallRail (число)
...
def callrail_call_webhook():
    ...
    if needs_followup:
        ...
        # Мгновенные Telegram-уведомления по каждому звонку отключены — Катрин хочет
        # только один сводный отчёт в конце дня (см. /daily-summary)
...
def callrail_outbound_call_webhook():
    ...
    # (мгновенное уведомление отключено, см. /daily-summary для сводки)
    if f["answered"]:
...
def performance_report():
    ...
    # Telegram-отправка отключена для этого отчёта — Катрин хочет только один
    # сводный отчёт в конце дня (см. /daily-summary). Данные всё равно пишутся в Summary.
...
def check_pending():
    ...
    for idx, row in enumerate(calls_ws.get_all_records(), start=2):
        ...
        if since_alert is not None and since_alert >= MISSED_CALL_ALERT_MINUTES:
            # Повторные Telegram-напоминания отключены — Катрин хочет только
            # один сводный отчёт в конце дня (см. /daily-summary)
    ...
    for idx, row in enumerate(messages_ws.get_all_records(), start=2):
        ...
        if since_alert is not None and since_alert >= SMS_NO_RESPONSE_ALERT_MINUTES:
            # Повторные Telegram-напоминания отключены — см. /daily-summary
...
def daily_summary():
    ...
    # Общая статистика по пропущенным звонкам за сегодня: сколько всего, сколько
    # успешно перезвонили, сколько всё ещё висит без ответа
    total_missed_today = sum(1 for c in today_calls if c["Status"] in ("Missed", "Voicemail"))
    still_pending_today = sum(
        1 for c in today_calls if c["Status"] in ("Missed", "Voicemail") and c["Follow-up Needed"] == "Yes"
    )
    called_back_today = total_missed_today - still_pending_today

    # Сортируем по booking rate (закрываемость) — от худшего к лучшему, как в общей таблице
    ordered = []
    for agent, s in stats.items():
        total_calls_agent = s["answered"] + s["missed"]
        answer_rate = round(s["answered"] / total_calls_agent * 100, 1) if total_calls_agent else 0
        booking_rate = round(s["appointments"] / s["answered"] * 100, 1) if s["answered"] else 0
        durations = [
            _duration_to_seconds(c["Duration"]) for c in today_calls
            if c["Dispatcher"] == agent and c["Status"] == "Answered"
        ]
        avg_duration = round(sum(durations) / len(durations)) if durations else 0
        # Простой сводный score: 60% вес на % ответов, 40% на закрываемость —
        # чтобы не отвечать на звонки было так же плохо, как не назначать встречи
        score = round(answer_rate * 0.6 + booking_rate * 0.4)
        ordered.append({
            "agent": agent, "s": s, "answer_rate": answer_rate, "booking_rate": booking_rate,
            "avg_duration": avg_duration, "total_calls": total_calls_agent, "score": score,
        })
    # Для рейтинга — от лучшего к худшему по score
    ranked = sorted(ordered, key=lambda x: x["score"], reverse=True)

    total_calls_company = sum(o["total_calls"] for o in ordered)
    total_answered_company = sum(o["s"]["answered"] for o in ordered)
    total_booked_company = sum(o["s"]["appointments"] for o in ordered)

    lines = [f"📊 Daily Report — {today}\n"]

    lines.append("🏆 Team Ranking (score = 60% answer rate + 40% booking rate)")
    for i, o in enumerate(ranked, start=1):
        lines.append(
            f"{i}. {o['agent']}: {o['score']}/100 — {o['total_calls']} calls, "
            f"{o['answer_rate']}% answered, {o['s']['appointments']} booked ({o['booking_rate']}%)"
        )
    lines.append("")
    ...
    if ranked:
        worst = ranked[-1]
        lines.append(
            f"⚠️ Zone of attention: {worst['agent']} — {worst['s']['missed']} missed calls, "
            f"lowest score ({worst['score']}/100). Worth checking in."
        )
        lines.append("")

    lines.append(
        f"🔻 Funnel: {total_calls_company} calls → {total_answered_company} answered → "
        f"{total_booked_company} appointments booked"
    )
    lines.append(
        f"Company-wide missed calls: {total_missed_today} "
        f"(called back: {called_back_today}, still pending: {still_pending_today})"
    )
    if not stats:
        lines.append("No data yet today.")

    # Пишем этот же отчёт (рейтинг, score, зона внимания, воронка) прямо в Google Таблицу —
    # отдельной вкладкой, которая перезаписывается каждый вечер свежими данными
    sheet_rows = [
        [f"DAILY REPORT — {today}", "", "", "", "", ""],
        ["Updated (Toronto time)", now_local().isoformat(sep=" ", timespec="seconds"), "", "", "", ""],
        ["", "", "", "", "", ""],
        ["TEAM RANKING", "", "", "", "", ""],
        ["#", "Dispatcher", "Score /100", "Calls", "Answer Rate", "Booked (Booking Rate)"],
    ]
    for i, o in enumerate(ranked, start=1):
        sheet_rows.append([
            i, o["agent"], o["score"], o["total_calls"], f"{o['answer_rate']}%",
            f"{o['s']['appointments']} ({o['booking_rate']}%)"
        ])
    sheet_rows.append(["", "", "", "", "", ""])
    if ranked:
        worst = ranked[-1]
        sheet_rows.append([
            "ZONE OF ATTENTION",
            f"{worst['agent']} — {worst['s']['missed']} missed calls, lowest score ({worst['score']}/100)",
            "", "", "", ""
        ])
    sheet_rows.append(["", "", "", "", "", ""])
    sheet_rows.append([
        "FUNNEL", f"{total_calls_company} calls → {total_answered_company} answered → "
                  f"{total_booked_company} appointments booked", "", "", "", ""
    ])
    sheet_rows.append([
        "MISSED CALLS", f"{total_missed_today} total — called back: {called_back_today}, "
                        f"still pending: {still_pending_today}", "", "", "", ""
    ])

    existing_sheets = [ws.title for ws in sh.worksheets()]
    if "Daily Report" not in existing_sheets:
        report_ws = sh.add_worksheet(title="Daily Report", rows=50, cols=6)
    else:
        report_ws = sh.worksheet("Daily Report")
        report_ws.clear()
    report_ws.update(values=sheet_rows, range_name="A1")
...
# ---------- Роут: опрос CallRail API напрямую (надёжнее вебхука для AI-тегов) ----------
# CallRail не всегда шлёт webhook "Call Modified", когда тег ставит сам ИИ (Conversion
# Signals) — это срабатывает надёжно только на ручные изменения. Поэтому вместо ожидания
# webhook мы сами регулярно СПРАШИВАЕМ у CallRail: "какие звонки за последние N часов
# помечены тегом про запись на встречу". Запускать по расписанию (cron) каждые 15-30 минут.
@app.route("/sync-callrail-tags", methods=["GET", "POST"])
def sync_callrail_tags():
    if not CALLRAIL_API_KEY or not CALLRAIL_ACCOUNT_ID:
        return jsonify({
            "status": "error",
            "note": "Не заданы переменные окружения CALLRAIL_API_KEY / CALLRAIL_ACCOUNT_ID"
        }), 400

    lookback_hours = int(request.args.get("hours", "6"))
    start_date = (now_local() - datetime.timedelta(hours=lookback_hours)).strftime("%Y-%m-%d")

    url = f"https://api.callrail.com/v3/a/{CALLRAIL_ACCOUNT_ID}/calls.json"
    params = {
        "tags[]": ["Schedule booked", "Schedule requested"],
        "start_date": start_date,
        "fields": "tags,customer_name,customer_phone_number,business_phone_number,start_time",
        "per_page": 250,
    }
    ...
    try:
        ...
        if resp.status_code >= 400:
            return jsonify({
                "status": "error",
                "note": f"CallRail API вернул {resp.status_code}",
                "callrail_response": resp.text[:1000],
            }), 502
        payload = resp.json()
    except requests.exceptions.RequestException as e:
        return jsonify({"status": "error", "note": f"Ошибка запроса к CallRail API: {e}"}), 502
    ...
    # Используем сырые значения (не get_all_records), чтобы не зависеть от того,
    # уникальны ли заголовки — берём первую колонку (job_id) во всех строках, кроме первой
    ...
    for c in calls:
        ...
        tags_text = ", ".join(str(t) for t in tags_list).lower()
        if "schedule booked" in tags_text:
            event_type = "callrail.schedule_booked"
        elif "schedule requested" in tags_text:
            event_type = "callrail.schedule_requested"
        else:
            continue

        customer_name = c.get("customer_name", "")
        customer_phone = c.get("customer_phone_number", "")
        destination = c.get("business_phone_number", "")
        dispatcher_name = DISPATCHER_PHONES.get(_normalize_phone(destination), "Unknown")

        # Используем реальное время звонка (а не момент синхронизации) — иначе старые
        # звонки, найденные с опозданием, задним числом попадают в "сегодня"
        raw_start = c.get("start_time", "")
        try:
            call_dt = datetime.datetime.fromisoformat(str(raw_start))
            if call_dt.tzinfo is not None:
                call_dt = call_dt.astimezone(TORONTO_TZ).replace(tzinfo=None)
            timestamp_created = call_dt.isoformat()
        except (ValueError, TypeError):
            timestamp_created = now_local().isoformat()

        new_rows.append([
            call_id, timestamp_created, "",
            customer_name, customer_phone, dispatcher_name, "",
            event_type, ""
        ])
        existing_ids.add(call_id)

    if new_rows:
        ws.append_rows(new_rows)  # один запрос к Google Sheets вместо множества отдельных

    return jsonify({"status": "ok", "calls_checked": len(calls), "appointments_added": len(new_rows)}), 200



# ---------- Роут: единая понятная таблица по диспетчерам (как просила Катрин) ----------
# Одна вкладка, без путаницы: звонки, ответы, встречи и главный показатель отрасли
# домашнего сервиса — Booking Rate (% звонков, ставших назначенной встречей).
@app.route("/dispatcher-table", methods=["GET"])
def dispatcher_table():
    period = request.args.get("period", "today")
    sh = get_sheet()
    ensure_worksheets(sh)
    now = now_local()
    ...
    appointments = [
        a for a in sh.worksheet("Appointments").get_all_records()
        if in_period(a["timestamp_created"])
        and a["event_type"] in ("job.scheduled", "job.appointment.scheduled", "job.appointment.rescheduled", "callrail.schedule_booked")
    ]
    ...
    for name in sorted(KNOWN_DISPATCHERS):
        ...
        durations = [_duration_to_seconds(c["Duration"]) for c in her_calls if c["Status"] == "Answered"]
        avg_duration = round(sum(durations) / len(durations)) if durations else 0
        booked = sum(1 for a in appointments if a["dispatcher"] == name)
        booking_rate = round(booked / answered * 100, 1) if answered else 0

        last_activity = None
        for c in her_calls:
            try:
                ts = datetime.datetime.fromisoformat(str(c["Timestamp"]).replace("Z", ""))
            except (ValueError, TypeError):
                continue
        ...
        if last_activity is None:
            ...
            is_workday = WORKDAY_START_HOUR <= now.hour < WORKDAY_END_HOUR
            status = f"⚠️ idle {idle_hours}h" if (is_workday and idle_hours >= NO_ACTIVITY_ALERT_HOURS) else "Active"

        rows.append({
            "name": name, "total": total, "answered": answered, "missed": missed,
            "answer_rate": answer_rate, "avg_duration": _format_duration(avg_duration),
            "booked": booked, "booking_rate": booking_rate, "status": status,
        })

    # Сортируем от худшего к лучшему по booking rate — главный показатель бизнеса
    rows.sort(key=lambda r: r["booking_rate"])

    sheet_rows = [[f"DISPATCHER PERFORMANCE — {label}", "", "", "", "", "", "", "", ""],
                  ["Updated (Toronto time)", now.isoformat(sep=" ", timespec="seconds"), "", "", "", "", "", "", ""],
                  ["", "", "", "", "", "", "", "", ""],
                  ["Dispatcher", "Calls Today" if period == "today" else "Calls (7d)", "Answered", "Missed",
                   "Answer Rate", "Avg Call Time", "Appointments Booked", "Booking Rate", "Status"]]
    for r in rows:
        sheet_rows.append([
            r["name"], r["total"], r["answered"], r["missed"], f"{r['answer_rate']}%",
            r["avg_duration"], r["booked"], f"{r['booking_rate']}%", r["status"]
        ])

    existing = [ws.title for ws in sh.worksheets()]
    if "Dispatcher Performance" not in existing:
        ws = sh.add_worksheet(title="Dispatcher Performance", rows=50, cols=9)
    else:
        ws = sh.worksheet("Dispatcher Performance")
        ws.clear()
    ws.update(values=sheet_rows, range_name="A1")

    return jsonify({"status": "ok", "period": label, "dispatchers": rows}), 200
