...
def housecallpro_appointment_webhook():
    ...
    if not appointment or appointment.get("id") is None:
        # ВРЕМЕННО: логируем полную структуру для событий без "appointment"
        # (например, job.created может иметь другую структуру с телефоном клиента)
        print(f"=== HCP OTHER EVENT ({event_type}) RAW PAYLOAD ===")
        print(data)
        print("=== END HCP OTHER EVENT RAW PAYLOAD ===")
        return jsonify({"status": "ok", "note": "тестовый запрос принят"}), 200
