...
def ensure_worksheets(sh):
    ...
    if "Daily History" not in existing:
        ws = sh.add_worksheet(title="Daily History", rows=2000, cols=6)
        ws.append_row([
            "Date", "Dispatcher", "Calls Answered", "Calls Missed",
            "SMS Answered", "Appointments Booked"
        ])
...
# ---------- Роут: CallRail Call Modified (реагируем на тег "Schedule booked") ----------
# Самый надёжный способ узнать, кто из диспетчеров назначил встречу — CallRail
# сам ставит тег "Schedule booked" на звонок, а мы уже знаем, кто на него отвечал
# (по номеру телефона, см. DISPATCHER_PHONES) — никаких догадок через Housecall Pro не нужно.
...
def callrail_call_modified_webhook():
    ...
    # Тег может прийти под разными названиями поля в зависимости от настроек CallRail —
    # проверяем несколько вариантов и ищем "schedule booked" без учёта регистра
    raw_tags = data.get("tags") or data.get("tag") or ""
    tags_text = str(raw_tags).lower()

    if "schedule booked" not in tags_text:
        return jsonify({"status": "ok", "note": "нет тега Schedule booked — пропущено"}), 200
    ...
    ws.append_row([
        ...
        "callrail.schedule_booked", ""
    ])

    return jsonify({"status": "ok", "dispatcher": dispatcher_name}), 200
...
def housecallpro_appointment_webhook():
    ...
    # Определяем диспетчера по недавнему звонку с этим же номером телефона —
    # логика: если клиент недавно звонил и с ним говорил конкретный диспетчер,
    # скорее всего именно он и назначил эту встречу
    dispatcher_name = _resolve_dispatcher_by_recent_call(
        sh.worksheet("Calls"), customer_phone, job_created_at
    ) or "Unknown"
...
def _resolve_dispatcher_by_recent_call(calls_ws, customer_phone, before_time_str, window_hours=720):
    """Ищет диспетчера по звонку с тем же номером телефона клиента за последние 30 дней
    (окно намеренно широкое — клиент мог звонить за несколько дней до самой записи),
    незадолго до создания этой работы — вероятно, именно он назначил встречу.
    Берём САМЫЙ ПОЗДНИЙ подходящий звонок в пределах окна (ближайший по времени к созданию job)."""
...
def daily_summary():
    ...
    for name in KNOWN_DISPATCHERS:
        stats.setdefault(name, {"answered": 0, "missed": 0, "sms_answered": 0, "sms_missed": 0, "appointments": 0})
    ...
    for agent, s in stats.items():
        history_ws.append_row([
            today, agent, s["answered"], s["missed"], s["sms_answered"], s["appointments"]
        ])
...
def dispatcher_report():
    ...
    rows = [
        ...
        ["", ""],
        ["Appointments Booked (all-time)", appointments_booked],
        ["Conversion Rate (appointments ÷ answered calls)", f"{conversion_rate}%"],
    ]
...
def company_overview():
    ...
    rows = [
        ...
        ["", ""],
        ["Appointments Booked", total_appointments],
        ["Company Conversion Rate (appointments ÷ answered calls)", f"{company_conversion}%"],
        ["", ""],
        ["PER-DISPATCHER BREAKDOWN", ""],
        ["Dispatcher", "Calls", "Answered", "Missed", "Answer Rate", "Appointments", "Conversion Rate"],
    ]
    ...
    for name in sorted(KNOWN_DISPATCHERS):
        ...
        per_dispatcher.append((name, t, a_, m_, ar, ap_count, cr))

    # Сортируем от худшего к лучшему по % ответов — проблемные сразу видны сверху
    per_dispatcher.sort(key=lambda r: r[4])
    for name, t, a_, m_, ar, ap_count, cr in per_dispatcher:
        rows.append([name, t, a_, m_, f"{ar}%", ap_count, f"{cr}%"])

    existing = [ws.title for ws in sh.worksheets()]
    if "Company Overview" not in existing:
        ws = sh.add_worksheet(title="Company Overview", rows=50, cols=7)
    else:
    ...
    return jsonify({
        ...
        "company_totals": {
            ...
            "answer_rate": company_answer_rate, "appointments": total_appointments,
            "conversion_rate": company_conversion,
        },
        "per_dispatcher": [
            {"name": n, "calls": t, "answered": a_, "missed": m_, "answer_rate": ar,
             "appointments": ap_count, "conversion_rate": cr}
            for n, t, a_, m_, ar, ap_count, cr in per_dispatcher
        ],
    }), 200
