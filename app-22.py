...
def sync_callrail_tags():
    ...
    params = {
        ...
        "fields": "tags,customer_name,customer_phone_number,business_phone_number,destinationnum",
        "per_page": 250,
    }
    ...
    # Не дублируем уже записанные звонки — собираем существующие job_id/call_id один раз
    existing_ids = {str(row.get("job_id", "")) for row in ws.get_all_records()}
    ...
    for c in calls:
        ...
        destination = c.get("destinationnum") or c.get("business_phone_number", "")
    ...
    return jsonify({"status": "ok", "calls_checked": len(calls), "appointments_added": added}), 200
